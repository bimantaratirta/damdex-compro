...
const Page = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const { data: events, loading: loadEvents } = useEvent({ limit: 5 });
  ...
  return (
    <AppLayout>
      {data && data.data.heroImageUrl && (
        ...
        >
          <Image alt="image1" priority unoptimized src={data?.data.heroImageUrl ?? ""} fill style={{ objectFit: "cover" }} />
        </Box>
      )}
      <Box sx={{ p: { xs: "0px 24px 24px", md: "0px 40px 40px", lg: 5 } }}>
        <Typography textAlign={"center"} mb={3} fontWeight={800} fontSize={{ lg: "60px", md: "50px", xs: "40px" }}>
          {language === "id" ? data?.data.titleIDN : data?.data.titleENG}
        </Typography>
        <Box sx={{ mb: 3 }}>
          <Stack direction={"row"} spacing={5} justifyContent={"start"}>
            <Typography width={"15vw"} fontWeight={800} fontSize={{ xs: "20px", lg: "26px" }}>
              Tempat
            </Typography>
            <Typography fontWeight={800} fontSize={{ xs: "20px", lg: "26px" }}>
              :
            </Typography>
            <Typography fontWeight={800} width={"50vw"} fontSize={{ xs: "20px", lg: "26px" }}>
              {language === "id" ? data?.data.eventVenueIDN : data?.data.eventVenueENG}
            </Typography>
          </Stack>
          <Stack direction={"row"} spacing={5} justifyContent={"start"}>
            <Typography width={"15vw"} fontSize={{ xs: "20px", lg: "26px" }} fontWeight={800}>
              Hari
            </Typography>
            <Typography fontWeight={800} fontSize={{ xs: "20px", lg: "26px" }}>
              :
        ...
        <Stack direction={"column"} fontSize={{ xs: "20px", lg: "26px" }} textAlign={"justify"}>
          <div
        ...
        <OtherListSection data={otherEvents} variant="Events" />
      </Box>
    </AppLayout>
  );
};
